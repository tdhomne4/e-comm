...
const AddressForm = (props) => {
  ...
  const inputContainer = {
   ...
   window.location.reload();
    ...
    if(props.address){
      ...
      setMobileNumber(props.address.mobileNumber);
      setPinCode(props.address.pinCode);
      setLocality(props.address.locality);
      setCityDistrictTown(props.address.cityDistrictTown);
      setState(props.address.state);
      setLandmark(props.address.landmark);
      setAlternatePhone(props.address.alternatePhone);
      setAddressType(props.address.addressType);
      setAddress(props.address.address);
      setId(props.address._id);
    } 
  }, [user.address]);
  ...
  const renderAddressForm = () => {
    ...
    return <div className="testing123">{renderAddressForm()}</div>;
  }
